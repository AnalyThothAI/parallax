"""Requeue News item briefs after lightweight brief contract cut."""

from __future__ import annotations

from alembic import op

revision = "20260601_0139"
down_revision = "20260531_0138"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute(_CLEAR_FAILED_CURRENT_BRIEFS_SQL)
    op.execute("ANALYZE news_item_agent_briefs")
    op.execute("ANALYZE news_projection_dirty_targets")


def downgrade() -> None:
    """No downgrade for clearing failed current brief read-model rows."""


_CLEAR_FAILED_CURRENT_BRIEFS_SQL = """
WITH runtime AS (
...
SELECT
  ...
  target.news_item_id,
  '',
  'news_item_brief_lightweight_contract',
