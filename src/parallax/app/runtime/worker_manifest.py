...
_WORKER_MANIFESTS: tuple[WorkerManifest, ...] = (
    ...
    WorkerManifest(
        ...
        kind=WorkerKind.AGENT_SIDE_EFFECT,
        runtime_constraint=WorkerRuntimeConstraint.DIRTY_TARGET_CONSUMER,
        worker_class="parallax.domains.news_intel.runtime.news_story_brief_worker.NewsStoryBriefWorker",
        start_priority=94,
        ...
        wakes_out=("news_story_brief_updated",),
    ),
    WorkerManifest(
    ...
    WorkerManifest(
        ...
        start_priority=80,
        input_contract=("macro_sync_windows", "configured macrodata history bundles"),
    ...
    WorkerManifest(
        ...
        start_priority=120,
        input_contract=("token radar read models", "news_page_rows", "watchlist read models"),
...
def _validate_worker_manifests() -> None:
    ...
    non_string_advisory_locks = {
        manifest.name: manifest.advisory_lock_key
        for manifest in _WORKER_MANIFESTS
        if manifest.advisory_lock_key is not None and type(manifest.advisory_lock_key) is not str
    }
    if non_string_advisory_locks:
        raise ValueError(f"non-string worker manifest advisory lock keys: {non_string_advisory_locks}")
    ...
    non_tuple_current_identity_columns = {
        ...
        for manifest in _WORKER_MANIFESTS
        if any(
            type(identity_columns) is not tuple
    ...
    non_string_current_identity_tables = {
        manifest.name: tuple(
            table_name
