"""Finalize News provider-rating agent gate."""
...
def upgrade() -> None:
    op.execute(
        f"""
        CREATE TEMP TABLE _news_provider_rating_gate_finalize_items ON COMMIT DROP AS
        WITH scored AS (
    ...
    op.execute(
        ...
        UPDATE news_items AS items
          ...
          FROM _news_provider_rating_gate_finalize_items AS gated,
               runtime_now
    ...
    op.execute(
        ...
        DELETE FROM news_projection_dirty_targets AS targets
         ...
         WHERE targets.projection_name = 'brief_input'
           ...
           AND items.agent_admission_status NOT IN ('eligible', 'eligible_refresh')
        """
    )
    op.execute(
        """
        ...
        SELECT 'page',
               ...
               md5(gated.news_item_id || ':news_provider_rating_gate_finalize:' || gated.gate_reason),
               GREATEST(
                   COALESCE(NULLIF(gated.fetched_at_ms, 0), 0),
                   COALESCE(NULLIF(gated.published_at_ms, 0), 0),
                   runtime_now.now_ms
               ),
               100,
               runtime_now.now_ms,
