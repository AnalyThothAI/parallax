...
def message_price_payload(row: dict[str, Any]) -> dict[str, Any]:
    tick_id = row.get("market_tick_id")
    if not tick_id:
        return {
    ...
    return {
        ...
        "observed_at_ms": row.get("market_tick_observed_at_ms"),
        "observation_lag_ms": lag,
        "observation_id": tick_id,
        "observation_kind": row.get("market_capture_method"),
    }
