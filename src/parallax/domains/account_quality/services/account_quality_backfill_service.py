...
class AccountQualityBackfillService:
    def __init__(self, *, repository: AccountQualityRepository) -> None:
        self.repository = repository

    def backfill_account_token_call_stats(self, *, limit: int) -> dict[str, Any]:
...
def _transaction(conn: Any) -> AbstractContextManager[Any]:
    try:
        ...
        raise RuntimeError("account_quality_backfill_transaction_required") from exc
    if not callable(transaction):
        raise RuntimeError("account_quality_backfill_transaction_required")
