...
- Provider token impacts and provider scores are evidence, not product truth by
  themselves. `market_scope` is metadata, not a rejection state. `NON_CRYPTO`
  identity classification remains valid outside News product gates, but News
  item brief and notification eligibility are market-wide. A bare ticker/common
