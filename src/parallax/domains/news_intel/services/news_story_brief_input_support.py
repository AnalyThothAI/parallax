...
def _entity_lane(row: Mapping[str, Any]) -> NewsStoryBriefEntityLane | None:
    ...
    return NewsStoryBriefEntityLane(
        entity_id=entity_id,
...
def _token_entity_lane(row: Mapping[str, Any]) -> NewsStoryBriefEntityLane | None:
...
def _fact_lane(row: Mapping[str, Any]) -> NewsStoryBriefFactLane:
    return NewsStoryBriefFactLane(
...
def _market_scope(
    *,
    ...
    entity_lanes: list[NewsStoryBriefEntityLane],
) -> list[str]:
...
def _optional_lane_scalar_list(row: Mapping[str, Any], field_name: str, *, lane_name: str) -> list[Any]:
    ...
    if value is None:
        return []
    if isinstance(value, str) or not isinstance(value, Sequence):
        raise ValueError(f"news_story_brief_{lane_name}_{field_name}_required")
    values = list(value)
...
__all__ = ["BODY_EXCERPT_MAX_CHARS", "MAX_ENTITY_LANES", "MAX_FACT_LANES"]
