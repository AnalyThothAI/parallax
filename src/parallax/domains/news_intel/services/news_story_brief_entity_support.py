...
def _source_backed_entity_key_support(packet: NewsStoryBriefInputPacket) -> _SourceBackedEntityKeySupport:
...
def _source_backed_domains(packet: NewsStoryBriefInputPacket) -> set[str]:
...
def _domain_proxy_source_keys(packet: NewsStoryBriefInputPacket, *, domain: str) -> set[str]:
    news_item = _packet_news_item(packet)
    keys: set[str] = set()
...
def _entity_lane_domains(entity: NewsStoryBriefEntityLane) -> set[str]:
    domains = {
...
def _translated_source_entity_keys(packet: NewsStoryBriefInputPacket) -> set[str]:
...
def _translated_entity_trigger_text(packet: NewsStoryBriefInputPacket) -> str:
    news_item = _packet_news_item(packet)
    texts = [
        news_item.title,
        news_item.summary,
