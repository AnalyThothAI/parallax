...
from parallax.domains.news_intel.services.news_story_brief_entity_support import (
...
class NewsStoryBriefValidationResult(BaseModel):
...
def validate_news_story_brief_output(
    *,
    payload: Any,
    packet: NewsStoryBriefInputPacket,
    audit: Any,
) -> NewsStoryBriefValidationResult:
    try:
        parsed = (
            payload if isinstance(payload, NewsStoryBriefPayload) else NewsStoryBriefPayload.model_validate(payload)
        )
    ...
    try:
        normalized = NewsStoryBriefPayload.model_validate(payload_dict)
    except ValidationError as exc:
    ...
    return NewsStoryBriefValidationResult(
        publishable=True,
...
def _evidence_ref_errors(payload: dict[str, Any], *, packet: NewsStoryBriefInputPacket) -> list[dict[str, str]]:
...
def _ready_evidence_errors(payload: dict[str, Any], *, packet: NewsStoryBriefInputPacket) -> list[dict[str, str]]:
...
def _drop_unsupported_market_impacts(payload: dict[str, Any], *, packet: NewsStoryBriefInputPacket) -> dict[str, Any]:
...
def _source_backed_market_labels(packet: NewsStoryBriefInputPacket) -> set[str]:
    labels: set[str] = set()
...
def _unsupported_entity_errors(payload: dict[str, Any], *, packet: NewsStoryBriefInputPacket) -> list[dict[str, str]]:
    errors: list[dict[str, str]] = []
...
def _packet_news_item(packet: NewsStoryBriefInputPacket) -> NewsStoryBriefNewsItem:
...
def _failed(errors: list[dict[str, str]]) -> NewsStoryBriefValidationResult:
    return NewsStoryBriefValidationResult(
        publishable=False,
        status="failed",
        payload=None,
...
__all__ = ["NewsStoryBriefValidationResult", "validate_news_story_brief_output"]
