...
def _admission_payload(*, item: Mapping[str, Any], admission: NewsItemAgentAdmission | None) -> dict[str, Any]:
    if admission is not None:
        if not isinstance(admission, NewsItemAgentAdmission):
            raise TypeError("news_story_agent_policy_admission_contract_required")
        return {
...
def _optional_policy_mapping(value: Any, field_name: str) -> dict[str, Any]:
    ...
    if isinstance(value, Mapping):
        return dict(value)
    raise ValueError(f"news_story_agent_policy_{field_name}_required")
