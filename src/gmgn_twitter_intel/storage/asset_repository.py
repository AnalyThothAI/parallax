...
class AssetRepository:
        ...
        rows = self.conn.execute(
            ...
            WITH recent_attention AS (
              SELECT
                ...
                MAX(token_intent_resolutions.decision_time_ms) AS latest_seen_ms,
                COUNT(*) FILTER (WHERE token_intent_resolutions.decision_time_ms >= %s) AS mentions_1h
              FROM token_intent_resolutions
              WHERE token_intent_resolutions.primary_venue_id IS NOT NULL
                AND token_intent_resolutions.resolution_status <> 'superseded'
                AND token_intent_resolutions.decision_time_ms >= %s
              GROUP BY token_intent_resolutions.primary_venue_id
            )
            SELECT
    ...
    def asset_timeline_rows(
        ...
        *,
        ...
        clauses = [
            "token_intent_resolutions.asset_id = %s",
            "events.received_at_ms >= %s",
            "token_intent_resolutions.resolution_status <> 'superseded'",
        ]
        ...
        rows = self.conn.execute(
            ...
            SELECT
              ...
              token_intent_resolutions.resolution_status AS attribution_status,
              token_intent_resolutions.confidence,
              assets.asset_type,
              assets.canonical_symbol,
              assets.identity_status,
              asset_venues.venue_id,
            ...
            JOIN assets ON assets.asset_id = token_intent_resolutions.asset_id
