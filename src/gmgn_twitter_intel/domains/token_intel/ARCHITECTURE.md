...
| Asset profile facts | `../asset_market/runtime/asset_profile_refresh_worker.py`, `../asset_market/runtime/token_profile_current_worker.py`, `../asset_market/services/token_profile_current_projection.py`, `../asset_market/read_models/token_profile_read_model.py` | `asset_profiles`, `cex_tokens`, `token_profile_current` | GMGN exact-token profile rows are source-cache facts. Public profile/icon facts come from `token_profile_current`, projected from persisted GMGN OpenAPI, GMGN stream exact snapshot, OKX DEX exact-address evidence, and static CEX icon facts on `cex_tokens`. Profile facts are never resolver evidence, ranking factors, or `factor_snapshot_json` fields. API and frontend code do not call providers. |
