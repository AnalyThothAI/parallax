...
| Market observation | `../asset_market/runtime/{asset_market_sync_worker.py,message_market_observation_worker.py}`, `../asset_market/services/{asset_market_sync.py,message_market_observation.py}`, `../asset_market/services/market_freshness_demand.py`, `../asset_market/repositories/price_observation_repository.py` | `cex_tokens`, `price_feeds`, `price_observations`, `current_market_field_facts`, `token_market_price_baselines`, `asset_identity_evidence/current` | CEX universe sync creates canonical CEX tokens and feeds. GMGN payload and message-level CEX / DEX quotes write price observations, field-aware current-market facts, and event price baselines. OKX DEX price sync prioritizes hot radar assets by missing/stale market facts before warm assets. Price freshness never stands in for identity confidence. |
| Radar projection | `runtime/token_radar_projection_worker.py`, `services/token_radar_projection.py`, `scoring/factor_snapshot.py`, `queries/token_radar_source_query.py`, `repositories/token_radar_repository.py` | `token_radar_rows.factor_snapshot_json`, `token_radar_publications`, `projection_runs`, `projection_offsets` | Projection rebuilds 5m / 1h / 4h / 24h windows for `all` and `matched` scopes, joins current resolutions with events, account profiles, enrichment labels, registry address identity, `asset_identity_current`, and `token_market_price_baselines`, then consumes `asset_market` current-market snapshots for scoring context and emits `token_factor_snapshot_v1`. Successful rebuilds atomically advance `token_radar_publications`; running/failed refreshes do not hide last published rows. It does not read latest price-observation rows as a live market contract, call providers, or preflight-refresh markets. |
...
Public Token Radar reads use `token_radar_publications` as the only current
version visibility pointer. `token_radar_projection_coverage` is not a public
read contract.
