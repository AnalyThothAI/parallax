...
| Radar projection | `runtime/token_radar_projection_worker.py`, `services/token_radar_projection.py`, `scoring/factor_snapshot.py`, `scoring/cross_section_normalizer.py`, `scoring/factor_diagnostics.py`, `queries/token_radar_target_feature_query.py` | `token_radar_dirty_targets`, `token_radar_target_features`, `token_radar_current_rows.factor_snapshot_json`, `token_radar_rank_history`, `token_radar_snapshot_audit`, `token_score_evaluations`, `projection_runs`, `projection_offsets` | Projection consumes dirty resolved targets, fetches target-scoped source rows, upserts compact target features, then ranks from `token_radar_target_features` into current/history/audit read models. There is no runtime full-window raw source scan and no dirty-empty catch-up scan. TokenRadarProjectionWorker is the only runtime writer for the Token Radar feature/current/history/audit read models; repair/audit commands may explicitly enqueue dirty targets from persisted facts. |
