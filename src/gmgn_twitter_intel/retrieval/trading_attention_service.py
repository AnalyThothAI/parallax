...
class TradingAttentionService:
    ...
    def pulse(
        ...
        *,
        ...
        token_links_by_event = self._token_links_by_event(event_ids)
    ...
    def _token_links_by_event(self, event_ids: list[str]) -> dict[str, list[dict[str, Any]]]:
        if not event_ids:
            return {}
        placeholders = ",".join("%s" for _ in event_ids)
        rows = self.signals.conn.execute(
            f"""
            SELECT event_id, identity_key, token_id, identity_status, chain, address, symbol,
                   source, attribution_status, attribution_confidence, attribution_weight
        ...
        for row in rows:
            ...
            by_event[event_id].append(_token_link(row, relation="direct"))

        mention_rows = self.signals.conn.execute(
            f"""
            SELECT event_id, identity_key, token_id, identity_status, chain, address, symbol, source
            FROM event_token_mentions
            WHERE event_id IN ({placeholders})
            ORDER BY token_id IS NULL, symbol
            """,
            event_ids,
        ).fetchall()
        for row in mention_rows:
            ...
            if (event_id, identity_key) in seen:
                continue
            by_event[event_id].append(_token_link(row, relation="keyword"))
...
def _token_link(row: Any, *, relation: str) -> dict[str, Any]:
    data = dict(row)
    status = str(data.get("identity_status") or ("resolved" if data.get("token_id") else "unresolved_symbol"))
    return {
        ...
        "relation": relation if data.get("token_id") else "keyword",
        "confidence": _float(data.get("attribution_confidence"), default=1.0 if data.get("token_id") else 0.35),
        "status": status,
...
def _kind(
    *,
    ...
    if any(link.get("token_id") for link in token_links):
        return "direct_token"
...
def _attention_keys(item: dict[str, Any]) -> list[str]:
    keys = [
        f"token:{link.get('token_id') or link.get('identity_key')}"
        for link in item.get("linked_tokens") or []
        if link.get("token_id") or link.get("identity_key")
    ]
