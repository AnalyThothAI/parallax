...
def _verify_gate_issues(root: Path, feature: SddFeature) -> list[str]:
    ...
    task_issues = _incomplete_task_issues(feature)
    return [_format_issue(issue) for issue in (*structural_issues, *evidence_issues)] + task_issues


def _incomplete_task_issues(feature: SddFeature) -> list[str]:
    incomplete_tasks = [task.title for task in feature.tasks if task.fields.get("status", "").strip().lower() != "[x]"]
    if not incomplete_tasks:
        return []
    return [
        "task-incomplete-in-verify-gate: "
        f"{feature.relative_path}/tasks.md: verify gate requires every task status [x]: " + ", ".join(incomplete_tasks)
    ]
