...
Admission, Token Profile Current, and Token Capture Tier dirty targets derive
`source_watermark_ms` only from positive current-row
...
| `news_item_brief_updated` | `NewsItemBriefWorker` | none; item briefs are audit-only after the story-current hard cut | `{count}` |
