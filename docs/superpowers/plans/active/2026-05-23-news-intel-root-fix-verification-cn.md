...
News root-fix implementation was built and running in Docker from the feature worktree image. During merge to `main`, News migrations were renumbered after the existing main Alembic chain: content classification is `20260523_0087`, filter indexes are `20260523_0088`, and `/news` renders live projected rows.

## Docker Rebuild Finding

`docker compose up -d --build` first produced an image that did not contain the news filter index migration; the container still had an older migration file from another branch. A forced `docker compose build --no-cache migrate app` exposed the reproducible root cause: the Python dependency stage could not build `quickjs==1.19.4` because `gcc` was missing.
...
Feature-worktree `alembic_version` before merge renumbering:

- `20260523_0086`

Merged-main Alembic order:

- `20260523_0085`: Token Radar storage root fix
- `20260523_0086`: Equity event runtime indexes
- `20260523_0087`: News content classification
- `20260523_0088`: News page filter indexes
