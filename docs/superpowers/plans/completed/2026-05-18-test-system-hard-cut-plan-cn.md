...
**Status:** Completed on 2026-05-18 in branch/worktree `codex/test-system-hard-cut`.

**Final Verification Evidence (2026-05-18):**

- `uv run pytest --collect-only -q`: 1436 tests collected.
- `uv run ruff check .`: passed.
- `uv run ruff format --check .`: 589 files already formatted.
- `uv run mypy src`: success, no issues in 370 source files.
- `make check-all`: passed.
- `make check-all` lane evidence:
  - Fast lane: 1156 passed, 2 skipped. Skips were `tests/unit/test_account_quality_service.py` environment data dependency and opt-in live provider drift.
  - Integration lane: 269 passed.
  - E2E lane: 5 passed, including `tests/e2e/test_backend_hot_path.py`.
  - Golden lane: 4 passed. `tests/golden/conftest.py` now provisions Postgres via testcontainers instead of silently skipping when the default local DSN is unavailable.
  - Coverage lane: 1435 passed, 1 skipped; total coverage 85.11%, above the 80.0% gate. The one remaining skip is `tests/contract/test_provider_drift_live.py`, intentionally opt-in via `GMGN_PROVIDER_DRIFT=1`.
- Hard-cut scan:
  - `rg 'GMGN_PROD_POSTGRES_DSN|_source_rows' tests`: no matches.
  - `rg '@pytest.mark.skip|pytest.skip|GMGN_PROD_POSTGRES_DSN|_source_rows|FakeRuntime|FakeRepository|without_postgres' tests`: only environment skip locations and unit-level public-surface fakes remain; integration fake runtime/repository usage is blocked by `tests/architecture/test_test_lane_contracts.py`.
...
- Move: `tests/unit/test_factor_snapshot.py`
  - Target: `tests/unit/test_factor_snapshot.py`
  - Keep as unit if it is pure schema/factor validation.
- Move: `tests/architecture/test_no_factor_snapshot_fallback.py`
...
- Move: `tests/unit/test_okx_dex_ws_client.py`
...
- Move: `tests/unit/test_pulse_decision_agent_client.py`
...
- [x] Write `test_no_root_pytest_files()` that fails if any `tests/test_*.py` exists.
- [x] Write `test_unit_tests_do_not_reference_live_dsns()` that scans `tests/unit/**/*.py` for `GMGN_PROD_POSTGRES_DSN`, `GMGN_TEST_POSTGRES_DSN`, and `connect_postgres_test`.
- [x] Write `test_business_skips_are_not_left_in_place()` that fails for `@pytest.mark.skip` and `pytest.skip(` outside allowlisted environment fixtures:
  - `tests/integration/conftest.py`
  - `tests/e2e/conftest.py`
  - `tests/postgres_test_utils.py`
  - `tests/contract/test_provider_drift_live.py`
- [x] Write `test_integration_tests_do_not_use_fake_runtime_repositories()` that scans `tests/integration/**/*.py` for `FakeRuntime`, `FakeRepository`, and `without_postgres`; allow only files moved to unit/contract.
- [x] Run: `uv run pytest tests/architecture/test_test_lane_contracts.py -q`.
- [x] Expected before cleanup: failures naming root tests, skipped tests, fake integration offenders.
- [x] Update `docs/TESTING.md` with the lane definitions and no-business-skip rule.
- [x] Update `Makefile` so `test-e2e` includes golden coverage or `check-all` calls a new `test-golden` target.
...
- [x] Move the loose plan file into `docs/superpowers/plans/active/`.
- [x] Remove or regenerate stale generated audit text that mentions `price_observations`.
- [x] Run: `uv run pytest tests/architecture/test_event_anchor_capture_redesign_contracts.py::test_old_price_observation_runtime_is_removed tests/architecture/test_harness_structure.py::test_lane_roots_have_no_loose_files -q`.
- [x] Expected after cleanup: both tests pass.

### Task 3: Move Root Tests Into Lanes

**Files:**
- Move: `tests/unit/test_factor_snapshot.py` -> `tests/unit/test_factor_snapshot.py`
- Move: `tests/architecture/test_no_factor_snapshot_fallback.py` -> `tests/architecture/test_no_factor_snapshot_fallback.py`
- Move: `tests/unit/test_okx_dex_ws_client.py` -> `tests/unit/test_okx_dex_ws_client.py`
- Move: `tests/unit/test_pulse_decision_agent_client.py` -> `tests/unit/test_pulse_decision_agent_client.py`

- [x] Move each file with `git mv`.
- [x] Update any imports or pytest node references in docs/plans if they point to old paths.
- [x] Run moved tests directly:
...
- [x] Run: `uv run pytest tests/architecture/test_test_lane_contracts.py::test_no_root_pytest_files -q`.
- [x] Expected: no root `tests/test_*.py` offenders.
...
- [x] Replace old registry symbol/name/decimals fixtures with current identity evidence/current seed helpers.
- [x] Replace skipped `pass` harness test with a real market-ready snapshot test.
- [x] Delete tech debt rows that only justify skipped tests after the tests are rewritten.
- [x] Run each rewritten file:
...
- [x] Run: `rg '@pytest.mark.skip|pytest.skip' tests/integration tests/unit`.
- [x] Expected: only environment skip locations remain.
...
- [x] Remove `GMGN_PROD_POSTGRES_DSN` support.
- [x] Remove monkeypatch of `_source_rows`.
- [x] Seed source facts through repositories in a disposable PostgreSQL database.
- [x] Run `TokenRadarProjectionWorker.rebuild_once()` twice with the same `now_ms`.
- [x] Assert `token_radar_rows` latest result is stable and no duplicate active rows are introduced.
- [x] Run: `uv run pytest tests/integration/test_token_radar_idempotency.py -q`.
...
- [x] Seed a resolved token, identity evidence/current, market tick, and enriched event without calling `WakeBus`.
- [x] Instantiate `TokenRadarProjectionWorker` with no wake waiter or an inert wake waiter.
- [x] Run `run_once(now_ms=...)`.
- [x] Assert `token_radar_projection_coverage.status == "ready"` and expected `token_radar_rows` exist.
- [x] Seed `token_radar_rows` without calling `notify_token_radar_updated`.
- [x] Run `PulseCandidateWorker.run_once_async(now_ms=...)` with fake decision provider.
- [x] Assert due pulse work is scanned from DB state.
- [x] Run: `uv run pytest tests/integration/test_worker_missed_wake_recovery.py -q`.
...
- [x] Use real `DBPoolBundle.acquire_advisory_lock_connection()` against test PostgreSQL.
- [x] Acquire the same advisory key twice from two worker instances.
- [x] Assert the second worker reports `WorkerResult(skipped=1, notes.reason="advisory_lock_unavailable")`.
- [x] Release the first lock.
- [x] Assert the second worker can acquire and process after release.
- [x] Run: `uv run pytest tests/integration/test_worker_advisory_lock_single_writer.py -q`.
...
- [x] If needed, change repository API to return actual inserted count or inserted tick ids.
- [x] If needed, change workers to emit wake only for actual inserted/coalesced changed targets.
- [x] Run: `uv run pytest tests/integration/test_market_tick_wake_idempotency.py tests/unit/test_market_tick_repository.py tests/unit/test_market_tick_poll_worker.py tests/unit/test_market_tick_stream_worker.py tests/unit/test_event_anchor_backfill_worker.py -q`.
...
- [x] Parse the Markdown table rows under `<!-- worker-inventory-keys: ... -->`.
- [x] Assert every `WorkersSettings` key appears exactly once.
- [x] Assert every `wakes_on` setting appears in the documented wake-in cell.
- [x] Assert every `WakeBus.notify_*` method appears in at least one documented wake-out cell.
- [x] Assert documented read-model writes match the single-writer allowlist in `test_worker_runtime_contracts.py`.
- [x] Run: `uv run pytest tests/architecture/test_worker_inventory_contract.py tests/architecture/test_worker_runtime_contracts.py -q`.
...
- [x] Add redacted provider fixtures with no secrets, tokens, or private account identifiers.
- [x] Test GMGN parser output: frame accepted, raw frame persist input exists, snapshot gate outcome correct, event normalized.
- [x] Test OKX WS parser output: price, liquidity, market cap, holders, observed time, raw payload retained.
- [x] Test OKX discovery parser output: chain/address/symbol/name/profile fields map to current provider candidate type.
- [x] Add live drift test guarded by `GMGN_PROVIDER_DRIFT=1`.
- [x] Run: `uv run pytest tests/contract/test_provider_protocol_fixtures.py -q`.
- [ ] Optional manual command: `GMGN_PROVIDER_DRIFT=1 uv run pytest tests/contract/test_provider_drift_live.py -q`.
  Not run in final verification; this is intentionally opt-in live-provider drift evidence, not CI evidence.
...
- [x] Build fake providers that implement the current provider protocols without network.
- [x] Bootstrap runtime against test PostgreSQL with fake providers and fake Pulse decision provider.
- [x] Feed a captured GMGN frame through `CollectorService.handle_frame`.
- [x] Run bounded worker convergence: market tick stream/poll or backfill, token radar projection, pulse candidate, notification rule/delivery.
- [x] Assert DB facts and read models at each step.
- [x] Assert HTTP endpoints read the resulting state.
- [x] Assert WS replay/subscription behavior for event, live market update, and notification.
- [x] Run: `uv run pytest tests/e2e/test_backend_hot_path.py -q`.
...
- [x] Run: `uv run pytest --collect-only -q`.
- [x] Run: `uv run ruff check .`.
- [x] Run: `uv run ruff format --check .`.
- [x] Run: `uv run mypy src`.
- [x] Run: `make check-all`.
- [x] Run: `rg '@pytest.mark.skip|pytest.skip|GMGN_PROD_POSTGRES_DSN|_source_rows|FakeRuntime|FakeRepository|without_postgres' tests`.
- [x] Record full verification output, skipped-test count, coverage, and e2e/golden result.
- [x] Move this plan to `docs/superpowers/plans/completed/` only after verification is recorded.
