...
- identity resolver 继续使用当前 `token_radar_v4_deterministic_resolver`，因为本阶段改的是 scoring/market/read contract，不重新定义 identity policy。
...
`src/gmgn_twitter_intel/pipeline/token_radar_projection.py` 当前 `_score()` 使用：
...
新增 `src/gmgn_twitter_intel/pipeline/token_radar_feature_builder.py`，只负责把 source rows 转成 scoring features。投影负责读数据和写 row，scoring 函数负责计算分数。
