...
`parallax` 已经有 GitHub dependency 的打包模式：
...
```text
src/parallax/integrations/coinglass/
```
...
```text
src/parallax/domains/asset_market/runtime/cex_derivatives_snapshot_worker.py
```
...
- 更新 `docs/ARCHITECTURE.md`、`src/parallax/domains/asset_market/ARCHITECTURE.md`、`docs/WORKERS.md`。
...
- Worker 默认关闭，需要 operator 在 `~/.parallax/workers.yaml` 显式开启。
