...
`marketlane-cli` 已经提供 quote access plane：`AsyncMarketlaneClient.quote()` 通过 market data gateway 取 quote；instrument registry 会为公开 symbol 动态合成 Yahoo-backed instrument，因此 AAPL/RKLB 这类普通 ticker 不需要预先写入 marketlane registry。证据见 `/Users/qinghuan/Documents/code/marketlane-cli/src/marketlane/client.py:63` 和 `/Users/qinghuan/Documents/code/marketlane-cli/src/marketlane/instruments/registry.py:524`。
