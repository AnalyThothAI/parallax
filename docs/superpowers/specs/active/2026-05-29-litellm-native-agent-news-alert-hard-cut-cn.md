...
当前 worker inventory 的 source of truth 是 `worker_manifest.py`。Agent lane 包括 Narrative `mention_semantics` 和 `token_discussion_digest`、News `news_item_brief`、Pulse `pulse_candidate`、Social `enrichment`、Watchlist `handle_summary`，其中 `narrative_admission` 不是 agent 但会向 narrative analysis 链路写 control-plane targets（`src/gmgn_twitter_intel/app/runtime/worker_manifest.py:315`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:335`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:361`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:448`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:593`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:636`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:662`）。

当前底层 LLM transport 仍由 `LLMGateway` 构造 `AsyncOpenAI`，并从 `agents` 包设置 tracing export key（`src/gmgn_twitter_intel/app/runtime/llm_gateway.py:7`, `src/gmgn_twitter_intel/app/runtime/llm_gateway.py:8`, `src/gmgn_twitter_intel/app/runtime/llm_gateway.py:15`, `src/gmgn_twitter_intel/app/runtime/llm_gateway.py:44`）。当前 structured JSON execution 通过 `client.chat.completions.create(..., response_format={"type": "json_object"})` 调用 OpenAI-compatible client（`src/gmgn_twitter_intel/integrations/openai_agents/structured_output_strategy.py:36`, `src/gmgn_twitter_intel/integrations/openai_agents/structured_output_strategy.py:59`）。

当前 `AgentExecutionGateway` 已经集中拥有 lane policy、global concurrency、RPM、capacity reservation、request audit、artifact hash、trace id、runtime version、client validation 等治理能力（`src/gmgn_twitter_intel/integrations/openai_agents/agent_execution_gateway.py:92`, `src/gmgn_twitter_intel/integrations/openai_agents/agent_execution_gateway.py:112`, `src/gmgn_twitter_intel/integrations/openai_agents/agent_execution_gateway.py:116`, `src/gmgn_twitter_intel/integrations/openai_agents/agent_execution_gateway.py:131`, `src/gmgn_twitter_intel/integrations/openai_agents/agent_execution_gateway.py:179`）。但 audit contract 仍把 backend 写成 `openai_agents_sdk`，runtime version 仍是 `agent-execution-plane-v1`（`src/gmgn_twitter_intel/platform/agent_execution.py:18`, `src/gmgn_twitter_intel/platform/agent_execution.py:151`, `src/gmgn_twitter_intel/platform/agent_execution.py:197`）。

当前 OpenAI provider composition root 把 Narrative、Pulse、Social、Watchlist、News 都接到 `OpenAIAgents*Client` 和 `AgentExecutionGateway`（`src/gmgn_twitter_intel/app/runtime/provider_wiring/openai.py:10`, `src/gmgn_twitter_intel/app/runtime/provider_wiring/openai.py:11`, `src/gmgn_twitter_intel/app/runtime/provider_wiring/openai.py:15`, `src/gmgn_twitter_intel/app/runtime/provider_wiring/openai.py:16`, `src/gmgn_twitter_intel/app/runtime/provider_wiring/openai.py:17`, `src/gmgn_twitter_intel/app/runtime/provider_wiring/openai.py:205`, `src/gmgn_twitter_intel/app/runtime/provider_wiring/openai.py:215`）。
...
News 当前链路已经是 Kappa/CQRS：`news_fetch` 写 provider items / canonical items，`news_item_process` 写 entities/token mentions/fact candidates，`news_story_projection` 写 story read models，`news_item_brief` 写 agent runs/current briefs，`news_page_projection` 写 serving rows（`src/gmgn_twitter_intel/app/runtime/worker_manifest.py:385`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:400`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:424`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:448`, `src/gmgn_twitter_intel/app/runtime/worker_manifest.py:468`）。

News dedup 已经存在 canonical item 和 observation edge 汇总：provider item upsert 维护 `news_item_observation_edges`，刷新 canonical item 的 duplicate observation count、source ids、source domains、provider article keys（`src/gmgn_twitter_intel/domains/news_intel/repositories/news_repository.py:1080`, `src/gmgn_twitter_intel/domains/news_intel/repositories/news_repository.py:1218`）。

News page projection 当前会把 provider signal、token lanes、fact lanes、agent brief compact envelope 放入 `news_page_rows`，并公开 `agent_status` 和 `agent_brief_computed_at_ms`（`src/gmgn_twitter_intel/domains/news_intel/services/news_page_projection.py:13`, `src/gmgn_twitter_intel/domains/news_intel/services/news_page_projection.py:48`, `src/gmgn_twitter_intel/domains/news_intel/services/news_page_projection.py:57`, `src/gmgn_twitter_intel/domains/news_intel/services/news_page_projection.py:60`）。但是 `_page_signal` 当前优先返回 provider signal；只有没有 provider signal 且 agent status ready 时才返回 agent signal（`src/gmgn_twitter_intel/domains/news_intel/services/news_page_projection.py:212`, `src/gmgn_twitter_intel/domains/news_intel/services/news_page_projection.py:217`, `src/gmgn_twitter_intel/domains/news_intel/services/news_page_projection.py:234`）。

News API 已经支持 `min_score` filter；它读取 `news_page_rows.signal_json ->> 'score'`，因此当前高分筛选实际主要筛 provider score（`src/gmgn_twitter_intel/app/surfaces/api/routes_news.py:17`, `src/gmgn_twitter_intel/app/surfaces/api/routes_news.py:24`, `src/gmgn_twitter_intel/domains/news_intel/repositories/news_repository.py:1504`, `src/gmgn_twitter_intel/domains/news_intel/repositories/news_repository.py:3380`）。

News item brief schema 已经具备中文 summary、market read、bull/bear view、affected assets、watch triggers、data gaps 和 no-execution-language 约束（`src/gmgn_twitter_intel/domains/news_intel/types/news_item_brief.py:63`, `src/gmgn_twitter_intel/domains/news_intel/types/news_item_brief.py:172`, `src/gmgn_twitter_intel/domains/news_intel/types/news_item_brief.py:189`）。但 `NewsItemBriefWorker` 当前遇到 provider signal 会直接 skip，所以 OpenNews provider-scored high-signal items 不会进入 agent brief 生成（`src/gmgn_twitter_intel/domains/news_intel/runtime/news_item_brief_worker.py:49`, `src/gmgn_twitter_intel/domains/news_intel/runtime/news_item_brief_worker.py:131`）。

Notifications 当前已经是 durable fact + delivery adapter 模型：rule engine 生成 candidates，worker insert notification facts 并 enqueue external deliveries，delivery worker 通过 Apprise 或 PushDeer 投递（`src/gmgn_twitter_intel/domains/notifications/services/notification_rules.py:36`, `src/gmgn_twitter_intel/domains/notifications/runtime/notification_worker.py:30`, `src/gmgn_twitter_intel/domains/notifications/runtime/notification_worker.py:82`, `src/gmgn_twitter_intel/domains/notifications/runtime/notification_delivery.py:21`, `src/gmgn_twitter_intel/domains/notifications/runtime/notification_delivery.py:65`）。当前 rule engine 没有 News rule，只评估 watched account、token flow、Signal Pulse（`src/gmgn_twitter_intel/domains/notifications/services/notification_rules.py:52`, `src/gmgn_twitter_intel/domains/notifications/services/notification_rules.py:64`）。

Notification repository 当前有 generic `dedup_key` 幂等插入，但 semantic duplicate 和 external cooldown 逻辑是 Pulse-specific（`src/gmgn_twitter_intel/domains/notifications/repositories/notification_repository.py:22`, `src/gmgn_twitter_intel/domains/notifications/repositories/notification_repository.py:49`, `src/gmgn_twitter_intel/domains/notifications/repositories/notification_repository.py:146`, `src/gmgn_twitter_intel/domains/notifications/repositories/notification_repository.py:173`）。

Pulse candidate gate 当前是 token/social/market read model 的独立产品门控，trade candidate 阈值为 72，high conviction 阈值为 78，不是 News provider score >85 的同一语义空间（`src/gmgn_twitter_intel/domains/pulse_lab/services/pulse_candidate_gate.py:42`, `src/gmgn_twitter_intel/domains/pulse_lab/services/pulse_candidate_gate.py:50`）。
