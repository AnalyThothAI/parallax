...
- **G3**：`app/runtime/providers_wiring.py` 是服务进程内**唯一**同时 import `gmgn_twitter_intel.integrations.*` 与 `gmgn_twitter_intel.domains.*.providers` 的文件。`app/surfaces/cli` 的一次性 ops 命令不纳入本 spec。验证：新增"服务装配点唯一性"测试通过。
