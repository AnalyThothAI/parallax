...
- **关联代码**:
  ...
  - `src/gmgn_twitter_intel/domains/pulse_lab/services/{agent_harness,agent_harness_eval}.py`
...
| 资源 | 位置 | 状态 |
|---|---|---|
| `pulse_agent_harness_versions` 表 | DB | ✅ 2 行（manifest 含 stages / framework / gate_policy / hard_blockers / eval_metadata），harness_hash 来自 `agent_harness.py` |
...
| `grade_pulse_deterministic_eval_case()` | `pulse_lab/services/agent_harness_eval.py:57` | ✅ 5 项检查：route 一致 / recommendation 一致 / hard_blocked→abstain / non_abstain 有 evidence-or-residual / critic ceiling / 无 trading 执行语 / 无失败 stage |
...
| `_factor_completeness` + `hard_blocked` 字段 | `pulse_candidate_worker.py` + `agent_harness.py` | ✅ 已实装 pre-LLM gate，cex 全部 hard-block 走 `research_only_gate` 不调 LLM |
...
| 主题 | 共识 | 本项目映射 |
|---|---|---|
| **Eval-driven** | outcome + trajectory + cost/latency 三类 grader；regression suite 防回退 | 现有 `agent_harness_eval` 是 outcome grader 雏形；缺 cost/latency grader 与"在 failed run 上也跑校验"路径 |
...
新建 `platform/observability/otel_setup.py`，按 OTel GenAI SemConv 给 `Runner.run` 加 span：`gen_ai.system / gen_ai.request.model / gen_ai.usage.input_tokens / cached_input_tokens / output_tokens / gen_ai.response.finish_reason`，并 attach 业务属性 `run_id / candidate_id / route / harness_hash / parse_mode / validation_passed`。OTLP 直发到本地 Langfuse v4（docker 起一个），不接外部云服务。pulse 三 stage 共享 trace_id 串成 pipeline trace。
...
新加命令：取最近 N 个 eval_case，让用户输入新 prompt 文件路径或新 model id，重跑 grader，diff 老 vs 新 pass rate 与 per-rule 违规数。基础设施都在（harness_hash + eval_case input_json + grader 函数），只缺 CLI wrapper。决策"prompt 改动是否 rollout"靠这条命令。
...
- **M1 schema 层 hard fix**：不动 DB schema。`harness_hash` 会因 `is_strict_json_schema=True` + jsonref 展平后的 schema 变化自动 bump 到新值（`agent_harness.py` 已基于 schema 内容算 hash），eval 表自动按新 harness 隔离记录。旧 harness_hash 的 eval_case/result 保留不删。
