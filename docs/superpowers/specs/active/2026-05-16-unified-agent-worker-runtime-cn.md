...
- **关联代码**:
  - `src/parallax/app/runtime/worker_base.py`、`worker_registry.py`、`worker_scheduler.py`
  ...
  - `src/parallax/integrations/openai_agents/{pulse_decision_agent_client,social_event_agent_client,watchlist_summary_agent_client,pulse_stage_prompts}.py`
  - `src/parallax/domains/pulse_lab/services/{agent_runtime,agent_eval}.py`
...
来源：`docs/WORKERS.md` + `app/runtime/worker_registry.py` + `~/.parallax/workers.yaml`。全部 `WorkerBase` 子类，由 `WorkerScheduler` 统一启停。
...
**S3. CLI: `parallax pulse eval-diff`**
...
**S4. Prompt 文件化**
迁 `pulse_stage_prompts.py` 的 f-string 模板到 `src/parallax/domains/pulse_lab/prompts/{analyst,critic,judge}_{cex,meme,research_only}.md`。`pulse_stage_prompt(route, stage)` 改为 `load_prompt(path).render(**context)`。git diff prompt 文件比 diff python f-string 易读 10×。
...
| S3 CLI | `parallax pulse eval-diff --since 7d --prompt-file=new.md` 输出 diff 表 |
