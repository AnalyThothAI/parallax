...
`closed_loop_harness` is currently a full domain module, not a small optional helper. It owns repository, read-model, runtime worker, scoring, settlement, credit, and snapshot-building code under `src/parallax/domains/closed_loop_harness/`. The live `EnrichmentWorker` imports `HarnessSnapshotBuilder` from that domain at `src/parallax/domains/social_enrichment/runtime/enrichment_worker.py:14`, calls it after the SocialEvent LLM run at `src/parallax/domains/social_enrichment/runtime/enrichment_worker.py:192`, and publishes a `harness_update` websocket payload at `src/parallax/domains/social_enrichment/runtime/enrichment_worker.py:117`.

The harness dependency is wired through the global runtime. `RepositorySession` imports `HarnessRepository` at `src/parallax/app/runtime/repository_session.py:19`, exposes it as `repos.harness` at `src/parallax/app/runtime/repository_session.py:71`, and constructs it at `src/parallax/app/runtime/repository_session.py:110`. Runtime bootstrap imports `HarnessRepository` at `src/parallax/app/runtime/bootstrap.py:24`, creates a pooled harness repository at `src/parallax/app/runtime/bootstrap.py:146`, and places it on both `runtime.harness` and `runtime.read_harness` at `src/parallax/app/runtime/bootstrap.py:191` and `src/parallax/app/runtime/bootstrap.py:197`.

The harness also has public and worker surfaces. `worker_registry.py` registers `harness_ops` at `src/parallax/app/runtime/worker_registry.py:31` and schedules it after handle summaries at `src/parallax/app/runtime/worker_registry.py:54`. `worker_factories/harness.py` imports `HarnessOpsWorker` at `src/parallax/app/runtime/worker_factories/harness.py:5` and constructs it at `src/parallax/app/runtime/worker_factories/harness.py:15`. The HTTP harness route imports `HarnessService` at `src/parallax/app/surfaces/api/routes_harness.py:13` and exposes `/social-events`, `/attention-seeds`, `/harness-snapshots`, `/harness-outcomes`, `/harness-credits`, `/harness-weights`, `/harness-health`, and `/harness-score-buckets` between `src/parallax/app/surfaces/api/routes_harness.py:18` and `src/parallax/app/surfaces/api/routes_harness.py:191`.

The event replay surface includes harness state inside normal event payloads. The recent-event API attaches `repos.harness.harness_for_event(event_id)` at `src/parallax/app/surfaces/api/routes_events.py:64` and batch attaches `harness_for_events` at `src/parallax/app/surfaces/api/routes_events.py:74`. WebSocket replay attaches the same field at `src/parallax/app/surfaces/api/ws.py:177`. Signal Pulse read-model health also reaches back into harness by passing a `harness` argument to `SignalPulseService` in `src/parallax/app/surfaces/api/routes_pulse.py:69` and `src/parallax/app/surfaces/api/routes_pulse.py:97`, then reports `settlement_coverage` from that dependency at `src/parallax/domains/pulse_lab/read_models/signal_pulse_service.py:55`.

Notification rules depend on harness snapshots as an alert source. `NotificationRuleEngine` accepts a `harness` dependency at `src/parallax/domains/notifications/services/notification_rules.py:44`, evaluates `_harness_snapshots` at `src/parallax/domains/notifications/services/notification_rules.py:66`, reads harness snapshots at `src/parallax/domains/notifications/services/notification_rules.py:322`, and emits notifications sourced from `harness_snapshots` at `src/parallax/domains/notifications/services/notification_rules.py:349`. The worker factory wires this with `HarnessService(repos.harness)` at `src/parallax/app/runtime/worker_factories/notifications.py:66`.

The storage model shows why this is a half-built closed loop rather than the production Pulse loop. The initial migration creates `social_event_extractions`, `attention_seeds`, `event_clusters`, `harness_snapshots`, `harness_decisions`, `harness_outcomes`, `harness_credits`, and `harness_weights` at `src/parallax/platform/db/alembic/versions/20260506_0001_initial_postgresql.py:340` through `src/parallax/platform/db/alembic/versions/20260506_0001_initial_postgresql.py:511`. `HarnessSnapshotBuilder` converts a SocialEventExtraction into shadow snapshots and decisions, not Pulse decisions, in `src/parallax/domains/closed_loop_harness/services/harness_snapshot_builder.py:36`. It records decisions with `execution_mode="shadow"` and `size=0.0` in `src/parallax/domains/closed_loop_harness/services/harness_snapshot_builder.py:239`, so this is not an executable or Pulse-gated decision loop.

Signal Pulse has its own agent ledger and eval path, but the naming also uses "harness". `PulseCandidateJobService` builds a pulse harness manifest at `src/parallax/domains/pulse_lab/services/pulse_candidate_job_service.py:110`, stores it through `repos.pulse_harness.upsert_agent_harness_version` at `src/parallax/domains/pulse_lab/services/pulse_candidate_job_service.py:127`, inserts eval cases at `src/parallax/domains/pulse_lab/services/pulse_candidate_job_service.py:270`, grades them at `src/parallax/domains/pulse_lab/services/pulse_candidate_job_service.py:276`, and then still upserts the public candidate at `src/parallax/domains/pulse_lab/services/pulse_candidate_job_service.py:282`. That means eval is currently telemetry, not a public write gate.

Pulse v2 still relies on agent tool behavior as part of correctness. `PulseAgentHarnessContract` says the default stages are `investigator` and `decision_maker` at `src/parallax/domains/pulse_lab/providers.py:42`, says investigator tools are `get_target_recent_tweets`, `get_target_price_action`, and `get_official_token_profile` at `src/parallax/domains/pulse_lab/providers.py:44`, and enables validators including `runtime_evidence_id_subset` at `src/parallax/domains/pulse_lab/providers.py:53`. `PulseDecisionRuntimeService` validates only whether supporting ids are in context or tool-contributed ids at `src/parallax/domains/pulse_lab/services/pulse_decision_runtime.py:81` and `src/parallax/domains/pulse_lab/services/pulse_decision_runtime.py:103`; it does not verify that the quoted event actually supports the claim.

The three LLM clients are not a unified runtime. Pulse has a strict flattened `_JsonOutputSchema` in `src/parallax/integrations/openai_agents/pulse_decision_agent_client.py:62`, Watchlist has a similar inline schema wrapper whose comment says it is waiting for a shared helper at `src/parallax/integrations/openai_agents/watchlist_summary_agent_client.py:41`, and SocialEvent directly passes `SocialEventPayload` as output type at `src/parallax/integrations/openai_agents/social_event_agent_client.py:73`. `provider_wiring/openai.py` constructs three separate OpenAI clients at `src/parallax/app/runtime/provider_wiring/openai.py:91`, `src/parallax/app/runtime/provider_wiring/openai.py:110`, and `src/parallax/app/runtime/provider_wiring/openai.py:141`.
...
- G1. Remove the `closed_loop_harness` domain from live runtime code: no `parallax.domains.closed_loop_harness` imports remain in `src/` after the cut.
...
- AC2. WHEN `uv run parallax --help` is regenerated THEN CLI SHALL not list `harness-snapshots`, `harness-outcomes`, `harness-credits`, `harness-weights`, `attribute-harness-credits`, or `update-harness-weights`.
