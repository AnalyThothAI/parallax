...
Twitter ws → events ───────────────────────────────────────────────┐
        ...
        extract_entities  ─►  build_token_evidence  ─►  build_token_intents
                  ...
                  upsert price_observation（gmgn_payload 自带价格时）
...
| message_payload / gmgn_payload | 6 |
...
| 「价格绑定写入有问题」 | ❌ 不成立 | `_insert_gmgn_payload_price_observation` 路径正确 |
...
- **证据**：`ingest_service.py:62-148` 一个 `transaction(self.evidence.conn)` 包住 events + event_entities + token_evidence + token_intents + registry_assets + token_intent_resolutions + token_intent_lookup_keys + price_observations + account_token_alerts + enrichment_jobs。
- **影响**：任意子环节失败（gmgn_payload `_raw_number` 抛 ValueError、registry 冲突）会让 events 行也丢失，调试 + retry 粒度受限。
- **修复方向**：A 段（事实必落）/ B 段（决策可重放）/ C 段（观测可选）三段提交，A 失败前序仍落库。
...
**T7 · IngestService 三段提交**
`IngestService.ingest_event` 在同一请求里按三个 commit boundary 顺序执行：
...
- **C 段 — Observations（原子，可选）**：gmgn_payload 触发的 price_observations。失败 A、B 已落；下次 sync 周期会补。

A commit 后 B 开始；B commit 后 C 开始。每段 idempotent on event_id。
...
```
collector → ingest:Stage A（事实） ──► ingest:Stage B（决策） ──► ingest:Stage C（观测）
...
| **GMGN payload 直供** | gmgn_token_payload 含 chain+address+mcap+price | `EXACT / CHAIN_ADDRESS_EXACT`，price_observations 有 `observation_kind=message_payload`、`source_event_id` 写入 | timeline post 的 `price.status=ready` |
