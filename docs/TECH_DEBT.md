...
|-------------|------------|------|----------|--------|-------|
| Repository `make check-all` baseline is red before frontend changes: final worktree stops at `ruff format --check` with 57 unchanged Python files after merging current `main`; a temporary mechanical format pass then exposed 101 mypy errors in 34 unchanged backend/worker files | pre-2026-05-22 baseline, detected by `2026-05-22-shadcn-frontend-system-hardening` | harness | high | Blocks the documented repository completion gate even when frontend gates pass; needs a backend typing/format cleanup separate from the shadcn frontend branch | unowned |
...
| `token_radar_rows` history had no bounded retention path because `listed_at_ms` depended on historical rows | pre-2026-05-20 Token Radar | 2026-05-20-token-radar-retention-watchlist-summary-hard-cut | Added `token_radar_target_first_seen`, projection-time first-seen writes, bounded first-seen backfill, and explicit `ops prune-token-radar` dry-run/execute maintenance |
