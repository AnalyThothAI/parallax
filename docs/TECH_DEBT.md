...
| Watchlist page still combines live `/api/recent` replay for sidebar counts with new handle-intel summary/timeline endpoints for the main panel | 2026-05-14-watchlist-handle-intel | web | medium | Counts can briefly diverge from the persisted handle timeline under replay lag; a future pass should move sidebar handle stats onto the same watchlist read model | unowned |
