...
|-------------|------------|------|----------|--------|-------|
| Repository `make check-all` baseline is red before frontend changes: final worktree stops at `ruff format --check` with 55 unchanged Python files; a temporary mechanical format pass then exposed 101 mypy errors in 34 unchanged backend/worker files | pre-2026-05-22 baseline, detected by `2026-05-22-shadcn-frontend-system-hardening` | harness | high | Blocks the documented repository completion gate even when frontend gates pass; needs a backend typing/format cleanup separate from the shadcn frontend branch | unowned |
