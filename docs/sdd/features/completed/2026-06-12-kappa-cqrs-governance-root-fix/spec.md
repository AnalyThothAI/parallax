...
> **Signal Pulse supersession (2026-07-21):** every Signal Pulse / Pulse Lab requirement, acceptance criterion, path, command, and target below is historical only and MUST NOT be implemented or restored. The completed hard-delete record is `docs/sdd/features/completed/2026-07-21-signal-pulse-hard-cut/`.

## Background

The audit recorded that /stocks-radar constructed StocksRadarService during a request and passed runtime.stock_quote_provider into it (docs/reviews/kappa-cqrs-worker-sql-audit-2026-06-12.md:7, docs/reviews/kappa-cqrs-worker-sql-audit-2026-06-12.md:11). It also recorded that the service loaded rows from PostgreSQL and then called self.quote_provider.quote(symbol) per symbol (docs/reviews/kappa-cqrs-worker-sql-audit-2026-06-12.md:12). The current target contract is a DB-only read path: public endpoints do not call providers, and Stocks Radar is served from stable current read models (`docs/CONTRACTS.md:51`, `docs/CONTRACTS.md:69`).
...
_rowcount(cursor) with the same non-boolean non-negative integer rowcount
contract
...
- G57. CEX Market Intel current/read-model writes require explicit repository-session or connection transactions for board rows, publication state, detail snapshots, and attempt-state writes, without naked `self.conn.commit()` compatibility in repository-owned commit paths.
...
- G80. PulseEvidenceBuilder sealed evidence packet construction uses formal evidence source repository methods for source events, enriched events, market facts, and identity facts, without optional method probes, Narrative projection dependencies, or empty-evidence compatibility defaults.
...
PulseEvidenceBuilder is the sealed evidence-packet boundary before Pulse LLM stages. It reads persisted source events, enriched events, market facts, and identity facts through formal evidence source repository methods. Missing repository methods are session/repository wiring failures and must fail before packet construction. Narrative admission/digest projections are not packet inputs.
...
- AC86. WHEN PulseEvidenceBuilder constructs a sealed evidence packet THEN system SHALL call formal evidence source repository methods directly for source events, enriched events, market facts, and identity facts, and SHALL not retain Narrative digest dependencies, optional method probes, or empty-evidence compatibility defaults for missing repository contracts.
...
- AC132. WHEN `DBPoolBundle` computes wake listener concurrency and wake pool size THEN system SHALL iterate manifest-declared wake listeners, SHALL read each wake worker's `enabled` and `concurrency` settings directly, SHALL reject `wakes_on` from runtime config, and SHALL not treat missing worker settings shape as zero wake listener demand.
...
- AC274. WHEN `NarrativeReadModel` hydrates Token Radar rows with `narrative_admission` THEN it SHALL require formal row identity fields `target_type` and `target_id` before admission lookup, SHALL report `status=missing` for rows without formal identity, and SHALL not retain legacy `type` / `id` target-identity compatibility.
...
- AC280. WHEN the backend Kappa/CQRS hard cut retires unused CEX derivative history THEN `CexDerivativeSeriesRepository`, its repository module, and its `RepositorySession` binding SHALL be absent, and migration `20260713_0183` SHALL drop `cex_derivative_series` without retaining a runtime compatibility accessor.
- AC281. WHEN the retired CEX derivative-history path is removed THEN its dedicated unit and architecture tests and current architecture/worker/contract descriptions SHALL also be removed; only migration history, the current hard-cut drop assertion, generated schema snapshots pending regeneration, and clearly historical review records may retain the old table name.
...
- AC302. WHEN live CEX repository ownership is composed THEN `RepositorySession` SHALL expose only the runtime-used `cex_oi_radar` and `cex_detail_snapshots` repositories, with no dynamic registration, alias, or test-only derivative-series seam.
- AC303. WHEN CEX hard-cut verification runs THEN the retained CEX domain and architecture suites SHALL pass without importing or reading a deleted derivative-series repository file, and targeted static checks SHALL find no current production reference to that repository.
...
- AC365. WHEN CEX read-model repositories account for `cex_oi_radar_rows` or `cex_detail_snapshots` write counts THEN they SHALL require PostgreSQL `cursor.rowcount` to be a non-boolean non-negative integer, missing, boolean, negative, or non-integer rowcount SHALL fail before write counts are returned, and repository code SHALL not restore malformed rowcount through `int(rowcount)` or `max(..., 0)` compatibility.
...
- AC388. WHEN `PulsePlaybooksRepository` writes Pulse playbook snapshots through `RETURNING` paths THEN PostgreSQL cursor rowcount SHALL be required, rowcount SHALL be valid 0/1 and match returned-row presence, snapshot no-change writes SHALL return no row only when rowcount is 0, changed snapshot writes SHALL require rowcount=1 with a returned row, missing or invalid rowcount SHALL fail before playbook rows are returned, and repository code SHALL not restore snapshot success through fallback `SELECT` or returned-row presence alone. The unused playbook-outcome writer/table SHALL remain removed; run outcomes belong to the Pulse run audit ledger.
...
- AC429. WHEN target-post responses are built THEN they SHALL preserve raw post evidence without Narrative semantic hydration, and `hydrate_target_posts`, `semantics_for_posts`, and `token_mention_semantics` reads SHALL not exist; AC448 supersedes the wider Narrative boundary.
...
- AC448. WHEN Token Radar or Token Case exposes Narrative state THEN the public field SHALL be `narrative_admission`, derived only from `narrative_admissions`, with status `admitted|suppressed|missing`, reason, `is_current`, computation time, minimal currentness, source/author coverage, and data gaps. Runtime SHALL NOT read or expose discussion digests, narrative deltas/clusters, per-post semantic placeholders, or Narrative digest data in Pulse evidence packets; removed Narrative semantic/digest tables and readers SHALL remain absent.
...
The July 2026 backend hard cut supersedes earlier acceptance criteria that
treated the following surfaces as current contracts: generic
`projection_dirty_ranges` and `projection_runs.dirty_ranges_written`,
`CexDerivativeSeriesRepository` / `cex_derivative_series`, token-factor
evaluation modules and storage, surrogate `snapshot_id` identity for current
read models, and the cross-domain `projects` abstraction. These surfaces are
deleted by migration `20260713_0183` and by the corresponding runtime hard cut.
Current projection work uses domain-owned durable queues; current read models
use stable product keys; CEX runtime composition exposes only repositories with
production callers. Earlier AC and verification paragraphs remain historical
evidence of the path to this decision and are not authority to restore the
deleted schema, repository seams, compatibility fields, or dedicated tests.
