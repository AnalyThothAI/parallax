...
|----------------------|--------|----------|
| AC1 - canonical schema | Pass | `uv run pytest tests/unit/test_postgres_schema.py -x` exited 0. |
| AC2 - nonempty upgrade | Pass | `uv run pytest tests/integration/test_postgres_schema_runtime.py -x` exited 0. |
| AC3 - live startup | Pass | `make docker-status` exited 0 with migration `0187` ready. |
...
closes this bounded migration repair.
