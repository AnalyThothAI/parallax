...
- **Token Radar currentness.** `/` is one full-height Radar and does not request
  ...
  Radar header has title/count/status on its first row and venue/window/scope
  controls on its second row. Its page-local status binds the exact current
  query identity. Content age is the browser clock minus
...
8. At tablet width around `834px`, confirm the desktop sidebar is hidden, the topbar trigger opens the shadcn drawer, drawer route navigation and topbar search still work, and the Radar two-row header, local status, full-height list, and no-overflow contract remain intact.
