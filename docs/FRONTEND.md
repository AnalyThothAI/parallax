...
- **Responsive CSS contract.** Mobile behavior is a tested architecture surface, not a best-effort visual tweak. Shell CSS owns `.cockpit-shell`, `.cockpit-grid`, `.center-column`, `.topbar`, `.desktop-side-rail`, `.responsive-control-panel`, `.mobile-task-nav`, `.mobile-route-nav`, and `[data-mobile-task-panel]` visibility. Feature CSS must not redefine those selectors or mobile task visibility hooks. Final shell breakpoint decisions live in `features/cockpit/ui/cockpitShellContract.css`.
- **Shell navigation.** The desktop side rail is desktop-only. Tablet and mobile users must still be able to reach Radar, Stocks, News, Macro, Watchlist, Ops, and Search from shell-owned navigation. The live Radar/Tape/Lab task switcher is mobile-only and separate from top-level route navigation.
