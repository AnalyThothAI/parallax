...
- **Responsive CSS contract.** Mobile behavior is a tested architecture surface, not a best-effort visual tweak. Shell CSS owns `.cockpit-shell`, `.cockpit-grid`, `.center-column`, `.topbar`, `.desktop-side-rail`, and `.mobile-route-nav`, split by owner files (`cockpitShell.css`, `CockpitTopbar.css`, `CockpitSideRail.css`, and `cockpitShellContract.css`). Final shell breakpoint decisions live in `features/cockpit/ui/cockpitShellContract.css`. Live-only task visibility is feature-owned by `features/live/ui/live.css`, using `.live-task-nav` and `[data-mobile-task-panel]` only inside `.live-page`.
...
- **Shell navigation.** The desktop side rail is desktop-only. Tablet and mobile users must still be able to reach Radar, Stocks, News, Macro, Watchlist, Ops, and Search from shell-owned navigation. The live Radar/Tape/Lab task switcher is `LivePage`-owned, mobile-only, and must not render on Stocks, News, Macro, Watchlist, Ops, Search, or Token Case routes.
...
6. Confirm token logos either load from `/api/token-images/{image_id}` or show
   ...
   GMGN `external-res`.
7. At `390px`, confirm the desktop side rail is hidden, the mobile route nav shows every top-level route without horizontal scrolling, Live-only Radar/Tape/Lab task switching works without route reload on `/`, Radar rows can scroll to the final row above the task nav without overlap, and non-Live routes do not render `.live-task-nav`.
8. At tablet width around `834px`, confirm the desktop side rail is hidden, the compact route nav is visible, the mobile Radar/Tape/Lab task nav is hidden, and route navigation still works.
