...
| `domains/token_intel/` | Token evidence, token intents, deterministic resolution, token-target views, Token Radar feature aggregation, `token_factor_snapshot_v2_alpha_gated` construction, factor-snapshot projection, evaluation diagnostics, audit queries, signal alerts. |
