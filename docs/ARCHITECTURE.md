...
| `domains/asset_market/` | Asset registry, chain/address identity, asset identity evidence/current identity selection, price observations, field-aware current market facts/read models, discovery, asset-market sync, message-market observation. |
| `domains/token_intel/` | Token evidence, token intents, deterministic resolution, token-target views, Token Radar feature aggregation, event price baselines, publication pointers, `token_factor_snapshot_v1` construction, factor-snapshot projection, audit queries, signal alerts. |
