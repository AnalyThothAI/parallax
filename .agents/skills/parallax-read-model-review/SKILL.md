...
description: Review or implement Parallax derived read model changes for stable keys, single writer ownership, idempotency, interval catch-up, and no compatibility shims. Use for "Read Model Change Review" tasks.
---

# Parallax Read Model Review

Use this skill for any change that touches a derived read model, projection writer, current-row identity, interval catch-up behavior, or public read path over projected state.
...
2. Name the fact source, read model, single runtime writer, public consumers, and catch-up cadence.
...
6. Confirm bounded `interval_seconds` catch-up re-reads PostgreSQL with no message-delivery dependency.
