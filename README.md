...
监听 GMGN 匿名公共 Twitter WebSocket，把可解析推文写入本地 SQLite WAL 数据库，并在 evidence、entity、signal 三层上提供 `/ws` 实时推送、HTTP 健康检查和 JSON CLI 查询。
...
GMGN public WS
  ...
  -> materialized alerts/windows
  -> /ws live push + replay
  -> CLI search / token-flow / account-alerts
```
...
WATCH_KEYWORDS=listing,airdrop,mainnet
```
...
| `WATCH_KEYWORDS` | 需要抽取并提醒的关键词，逗号分隔 | 空 |
...
uv run gmgn-twitter-intel keyword-flow --window 1h --limit 20
...
- `WATCH_KEYWORDS` 决定 keyword entity 和 account keyword alert。
- CA、cashtag、hashtag、mention、URL/domain、keyword 都是确定性抽取。
