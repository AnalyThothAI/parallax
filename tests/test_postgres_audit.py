...
def test_operational_audit_reports_counts_fk_checks_and_projection_schema(tmp_path):
    ...
    assert payload["migration_version"] == "20260506_0004"
    assert payload["counts"]["events"] == 0
    assert payload["projection_schema"]["projection_offsets"] is True
    assert payload["projection_schema"]["token_flow_window_snapshots"] is True
    assert payload["foreign_key_checks"]["event_token_attributions_missing_events"] == 0
...
def test_query_audit_explains_hot_read_paths_without_analyze(tmp_path):
    ...
    assert {"recent_all", "search_fts", "token_flow_5m_shape", "token_posts_recent"}.issubset(names)
