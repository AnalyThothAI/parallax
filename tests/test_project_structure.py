...
def test_legacy_root_runtime_files_are_removed():
    ...
    assert not (ROOT / "deploy" / "systemd" / "gmgn-twitter-cli.service").exists()
    assert not (ROOT / "deploy" / "macos" / "install_launchd.sh").exists()
    assert not (ROOT / "deploy" / "nginx" / "gmgn-twitter-cli.conf").exists()
    assert not (ROOT / "src" / "gmgn_twitter_cli" / "service_control.py").exists()
