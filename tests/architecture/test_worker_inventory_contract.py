...
@pytest.mark.architecture
def test_worker_manifest_validation_rejects_non_string_read_model_identity_tables(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    first_writer_index = next(
    ...
    manifests[first_writer_index] = replace(
        first_writer,
        current_read_model_identities=(
            (123, identity_columns),
            *first_writer.current_read_model_identities[1:],
        ),
    )
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="non-string current read model identity tables"):
        worker_manifest_module._validate_worker_manifests()


@pytest.mark.architecture
def test_worker_manifest_validation_rejects_non_tuple_read_model_identity_columns(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    first_writer_index = next(
    ...
    manifests[first_writer_index] = replace(
        first_writer,
        current_read_model_identities=(
            (table_name, list(identity_columns)),
            *first_writer.current_read_model_identities[1:],
        ),
    )
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="non-tuple current read model identity columns"):
        worker_manifest_module._validate_worker_manifests()


@pytest.mark.architecture
def test_worker_manifest_validation_rejects_non_string_read_model_identity_columns(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    first_writer_index = next(
    ...
    with pytest.raises(ValueError, match="non-string current read model identity columns"):
        worker_manifest_module._validate_worker_manifests()
...
@pytest.mark.architecture
def test_worker_manifest_validation_rejects_malformed_read_model_identity_entries(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    first_writer_index = next(
    ...
    manifests[first_writer_index] = replace(
        first_writer,
        current_read_model_identities=(
            (table_name, identity_columns, "unexpected_compat_field"),
            *first_writer.current_read_model_identities[1:],
        ),
    )
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="malformed current read model identity entries"):
        worker_manifest_module._validate_worker_manifests()
...
def test_worker_manifest_validation_rejects_non_string_queue_depth_tables(
    ...
    manifests = list(all_worker_manifests())
    first_queue_index = next(index for index, manifest in enumerate(manifests) if manifest.queue_depth_table)
    manifests[first_queue_index] = replace(manifests[first_queue_index], queue_depth_table=123)
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="non-string worker manifest queue depth tables"):
        worker_manifest_module._validate_worker_manifests()
...
@pytest.mark.architecture
def test_worker_manifest_validation_rejects_provider_schedulers_with_dirty_targets(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    first_provider_index = next(
    ...
    manifests[first_provider_index] = replace(
        ...
        writes_control_plane=("provider_dirty_targets",),
        dirty_target_tables=("provider_dirty_targets",),
    )
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="bounded provider scheduler manifests declaring dirty target tables"):
        worker_manifest_module._validate_worker_manifests()


@pytest.mark.architecture
def test_worker_manifest_validation_rejects_provider_schedulers_with_queue_depth(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    first_provider_index = next(
    ...
    manifests[first_provider_index] = replace(
        manifests[first_provider_index],
        writes_control_plane=("provider_jobs",),
        queue_depth_table="provider_jobs",
    )
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="bounded provider scheduler manifests declaring queue depth tables"):
        worker_manifest_module._validate_worker_manifests()
...
@pytest.mark.architecture
def test_worker_manifest_validation_rejects_non_string_advisory_lock_keys(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    first_locked_index = next(index for index, manifest in enumerate(manifests) if manifest.advisory_lock_key)
    manifests[first_locked_index] = replace(manifests[first_locked_index], advisory_lock_key=123)
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="non-string worker manifest advisory lock keys"):
        worker_manifest_module._validate_worker_manifests()
...
def test_worker_manifest_validation_rejects_missing_factory_modules(
    ...
    manifests = list(all_worker_manifests())
    manifests[0] = replace(manifests[0], factory="missing_factory.py")
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="missing worker manifest factory modules"):
        worker_manifest_module._validate_worker_manifests()


@pytest.mark.architecture
def test_worker_manifest_validation_rejects_duplicate_worker_classes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    manifests[1] = replace(manifests[1], worker_class=manifests[0].worker_class)
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="duplicate worker manifest classes"):
        worker_manifest_module._validate_worker_manifests()
...
@pytest.mark.architecture
def test_worker_manifest_validation_rejects_missing_worker_class_names(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    manifests[0] = replace(
        manifests[0],
        worker_class="parallax.domains.ingestion.runtime.collector_service.MissingWorker",
    )
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="missing worker manifest class names"):
        worker_manifest_module._validate_worker_manifests()
...
def test_worker_manifest_validation_rejects_negative_start_priorities(
    ...
    manifests = list(all_worker_manifests())
    manifests[0] = replace(manifests[0], start_priority=-1)
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="negative worker manifest start priorities"):
        worker_manifest_module._validate_worker_manifests()
...
@pytest.mark.architecture
def test_worker_manifest_validation_rejects_non_tuple_contract_fields(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    manifests[0] = replace(manifests[0], input_contract=["gmgn public websocket frames"])
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="non-tuple worker manifest contract fields"):
        worker_manifest_module._validate_worker_manifests()
...
def test_worker_manifest_validation_rejects_blank_input_contracts(
    ...
    manifests = list(all_worker_manifests())
    manifests[0] = replace(manifests[0], input_contract=(*manifests[0].input_contract, "   "))
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="blank worker manifest input contracts"):
        worker_manifest_module._validate_worker_manifests()
...
def test_worker_manifest_validation_rejects_empty_ordering_keys(
    ...
    manifests = list(all_worker_manifests())
    manifests[0] = replace(manifests[0], ordering_keys=())
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="worker manifests missing ordering keys"):
        worker_manifest_module._validate_worker_manifests()


@pytest.mark.architecture
def test_worker_manifest_validation_rejects_blank_ordering_keys(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifests = list(all_worker_manifests())
    manifests[0] = replace(manifests[0], ordering_keys=(*manifests[0].ordering_keys, "   "))
    monkeypatch.setattr(worker_manifest_module, "_WORKER_MANIFESTS", tuple(manifests))

    with pytest.raises(ValueError, match="blank worker manifest ordering keys"):
        worker_manifest_module._validate_worker_manifests()
