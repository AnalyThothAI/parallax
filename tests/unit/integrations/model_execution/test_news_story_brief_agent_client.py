...
def test_news_story_brief_client_executes_strict_payload_with_caller_reservation() -> None:
    ...
    result = asyncio.run(client.brief_story(run_id="run-1", packet=packet, reservation=reservation))
    ...
    assert result == {
        ...
        "agent_run_audit": {"status": "done", "lane": NEWS_STORY_BRIEF_LANE},
    }
...
def _packet() -> NewsStoryBriefInputPacket:
    return NewsStoryBriefInputPacket(
        packet_id="packet-1",
