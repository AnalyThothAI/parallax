...
from parallax.domains.news_intel.services.news_story_agent_policy import news_story_brief_priority
...
def test_news_story_brief_priority_ignores_provider_scores() -> None:
    assert news_story_brief_priority(
...
def test_news_story_brief_priority_ignores_provider_ready_status() -> None:
    assert news_story_brief_priority(
        item=_item(provider_signal_json={"source": "provider", "status": "ready"}),
    ) == news_story_brief_priority(item=_item(provider_signal_json={}))


def test_news_story_brief_priority_prefers_refresh_and_material_delta() -> None:
    regular = news_story_brief_priority(
        item=_item(agent_admission_json={"status": "eligible", "reason": "eligible"}),
    )
    material = news_story_brief_priority(
        item=_item(
    ...
    refresh = news_story_brief_priority(
...
def test_news_story_brief_priority_ignores_sidecar_material_delta_without_admission_basis() -> None:
    assert news_story_brief_priority(
        item=_item(
            agent_admission_json={"status": "eligible", "reason": "eligible"},
...
def test_news_story_brief_priority_does_not_restore_status_from_scalar_alias() -> None:
    assert (
        news_story_brief_priority(
            item=_item(
...
def test_news_story_brief_priority_does_not_restore_scope_from_legacy_aliases() -> None:
    alias_priority = news_story_brief_priority(
        item=_item(
            agent_admission_json={"status": "eligible", "reason": "eligible"},
            market_scope=["crypto", "macro_rates", "us_equity"],
        ),
    )
    nested_alias_priority = news_story_brief_priority(
        item=_item(
            agent_admission_json={"status": "eligible", "reason": "eligible"},
            market_scope_json={"market_scope": ["crypto", "macro_rates", "us_equity"]},
        ),
    )
    formal_priority = news_story_brief_priority(
        item=_item(
            agent_admission_json={"status": "eligible", "reason": "eligible"},
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            "news_story_agent_policy_agent_admission_json_required",
            id="admission_string",
        ),
        pytest.param(
            {"agent_admission_json": {"status": "eligible", "basis": '{"material_delta":true}'}},
            "news_story_agent_policy_basis_required",
            id="basis_string",
        ),
        pytest.param(
            {
            ...
            "news_story_agent_policy_material_delta_changed_fields_required",
            id="changed_fields_string",
        ),
        pytest.param(
            {"agent_admission_json": {"status": "eligible"}, "market_scope_json": "crypto"},
            "news_story_agent_policy_market_scope_json_required",
            id="market_scope_string",
        ),
        pytest.param(
            {"agent_admission_json": {"status": "eligible"}, "market_scope_json": {"scope": "crypto"}},
            "news_story_agent_policy_market_scope_json_scope_required",
            id="market_scope_scope_string",
        ),
    ],
)
def test_news_story_brief_priority_rejects_malformed_present_policy_fields(
    overrides: dict[str, object],
    match: str,
) -> None:
    with pytest.raises(ValueError, match=match):
        news_story_brief_priority(item=_item(**overrides))


def test_news_story_brief_priority_uses_runtime_admission_basis() -> None:
    priority = news_story_brief_priority(
        item=_item(),
...
def test_news_story_brief_priority_rejects_loose_runtime_admission_object() -> None:
    ...
    with pytest.raises(TypeError, match="news_story_agent_policy_admission_contract_required"):
        news_story_brief_priority(item=_item(), admission=LooseAdmission())


def test_news_story_brief_priority_prefers_authoritative_multi_scope_items() -> None:
    ordinary = news_story_brief_priority(
        item=_item(
    ...
    authoritative = news_story_brief_priority(
        item=_item(
...
def test_news_story_brief_priority_deprioritizes_non_executable_statuses() -> None:
    assert (
        news_story_brief_priority(
