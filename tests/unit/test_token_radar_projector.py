...
@pytest.mark.parametrize("field", ["raw_composite_score", "gates_max_decision"])
...
def test_private_cache_pruning_uses_one_explicit_transaction_and_shared_budget() -> None:
    ...
    result = projector.prune_private_cache(
        windows=("5m",),
        scopes=("all",),
        now_ms=1_000_000,
...
@pytest.mark.parametrize(
    ("row", "venue"),
    [
        ({"target_type": "CexToken", "asset_chain_id": "eip155:56"}, "cex"),
        ({"target_type": "Asset", "asset_chain_id": "eip155:56"}, "bsc"),
        ({"target_type": "Asset", "factor_snapshot_json": {"subject": {"chain": "base"}}}, "base"),
    ],
)
