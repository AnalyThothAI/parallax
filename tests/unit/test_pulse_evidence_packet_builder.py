...
def test_packet_has_no_legacy_narrative_projection_context() -> None:
    context = _context(target_type="chain_token", target_id="solana:abc", source_event_ids=[])
    repo = FakeEvidenceSourceRepository()

    packet = _builder(repo).build(context, run_id="run-no-narrative-projection", now_ms=NOW_MS)
    gate = EvidenceCompletenessGate().evaluate(packet)

    assert gate.max_decision_status == "abstain"
    assert gate.blocked_reason == "blocked_social_contract"
    assert "discussion_digest" not in packet.admission_context
    assert "discussion_digest" not in packet.summary_json
    assert "discussion_digest_id" not in packet.source_fingerprints
