...
export function getNotifications(token: string, unreadOnly = false): Promise<ApiResponse<NotificationsData>> {
...
export function markNotificationRead(
  ...
  return postApi<{ notification_id: string; updated: boolean }>(`/api/notifications/${notificationId}/read`, { token });
}

export function markAllNotificationsRead(token: string): Promise<ApiResponse<{ updated_count: number }>> {
