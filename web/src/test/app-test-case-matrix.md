...
| 659 | uses Signal Lab as the trader-facing product label | L1 | Shell/navigation component copy |
| 667 | keeps Signal Lab Pulse visible in the live cockpit and opens the shared inspector | L2 | Live cockpit plus pulse inspector |
| 691 | routes Signal Pulse notifications into Signal Lab instead of token search | L2 | Notification route integration |
| 712 | queries the compact Signal Lab Pulse as a fresh live feed | L2 | Query params/API contract |
| 731 | switches the left rail into the Signal Lab workbench without losing the selected pulse item | L2 | Signal Lab route state |
...
| 795 | uses watchlist clicks as a Signal Lab account lens | L2 | Watchlist-to-Signal-Lab routing |
| 823 | shows watched account events when an account lens has no Signal Pulse candidates | L2 | Signal Lab account fallback |
| 861 | renders the selected token drawer with the mock structure and no extra override controls | L2 | Live selected-token drawer |
| 884 | opens Timeline by default, requests timeline/posts, and keeps token Lab as a pointer into Signal Lab | L2 | Token detail integration |
...
| 1019 | drives selected token detail by production windows instead of manual timeline buckets | L2 | Token detail route/API integration |
...
| 1113 | removes narrative product surface and exposes signal lab entry points | L1 | Shell/live component contract |
| 1121 | uses the Signal Pulse read model as the only Signal Lab product source in the UI | L2 | Signal Lab data source integration |
| 1137 | keeps settlement horizons out of the trader-facing Signal Lab and global token radar rail | L1 | Signal Lab UI copy/surface |
| 1152 | routes Signal Lab toolbar filters into the Signal Pulse read model | L2 | Signal Lab URL/API state |
| 1182 | uses the Signal Lab cursor without duplicating aggregate summary or overlapping rows | L2 | Signal Lab pagination |
| 1245 | keeps live compact pulse selection independent from workbench status filters | L2 | Live/Signal Lab state isolation |
| 1269 | renders Signal Pulse rows and opens the right-side inspector | L2 | Signal Lab route integration |
...
| 1337 | requests selected token detail by target identity | L2 | Token detail API params |
| 1358 | does not offer an audit page for unresolved token radar rows | L2 | Unresolved target routing |
| 1369 | realigns the drawer when the selected token disappears after a window switch | L2 | Live route selection state |
...
| 1418 | uses the Signal Lab mobile task when cold-loading a Signal Lab route | L2 | Signal Lab mobile route state |
| 1430 | keeps mobile radar token clicks on the detail drawer without changing the search input | L2 | Mobile live interaction |
| 1450 | switches mobile tasks without resetting window, scope, or selected token | L2 | Mobile route/local state split |
| 1479 | returns mobile Radar and Tape tasks to the live cockpit after opening Signal Lab | L2 | Mobile shell route ownership |
...
| 1529 | renders Signal Lab pulse rows without nested source controls | L1 | Signal Lab component contract |
