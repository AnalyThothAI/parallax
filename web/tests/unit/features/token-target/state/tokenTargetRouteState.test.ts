...
describe("tokenTargetRouteState", () => {
  ...
  it("accepts supported window, scope, range, and sort params", () => {
    expect(
      parseTokenTargetRouteState(
        new URLSearchParams("window=24h&scope=matched&postRange=since_ignition&postSort=catalyst"),
      ),
    ).toEqual({
      window: "24h",
      scope: "matched",
  ...
  it("serializes non-default params in stable order", () => {
    ...
    ).toBe("window=4h&scope=matched&postRange=all_history&postSort=quality");
  });
});
